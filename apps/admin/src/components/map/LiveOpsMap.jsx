...
const DEFAULT_CENTER = [18.559, 73.8];
function FitOnce({
  points
}) {
  const map = useMap();
...
export function LiveOpsMap({
  ...
  const driverPoints = useMemo(() => drivers.filter(d => !isZeroPoint(d.currentLocation)).map(d => toLeafletTuple(d.currentLocation)), [drivers]);
  ...
  return <div className={className} style={{
    height
      ...
      <MapContainer center={DEFAULT_CENTER} zoom={12} style={{
      height: '100%',
      width: '100%',
      borderRadius: 'inherit'
        ...
        {drivers.filter(d => !isZeroPoint(d.currentLocation)).map(d => <Marker key={d.id} position={toLeafletTuple(d.currentLocation)} icon={carDivIcon(d.status, d.heading ?? 0)} eventHandlers={onSelectDriver ? {
        ...
        {pickups.filter(p => !isZeroPoint(p.coordinates)).map(p => <Marker key={p.id} position={toLeafletTuple(p.coordinates)} icon={pinDivIcon('#7c3aed', true)}>
              <Popup>
                <div className="text-xs">
