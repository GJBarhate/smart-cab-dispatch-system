...
import { LayoutDashboard, Map, Users, UserRound, Inbox, Kanban, ListOrdered, Gauge, BarChart3, Settings, LogOut, WifiOff } from 'lucide-react';
...
export function AdminShell() {
  ...
  return <div className="flex h-full min-h-screen bg-canvas">
      ...
      <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-3 focus:top-3 focus:z-[1000] focus:rounded-md focus:bg-ops-600 focus:px-3 focus:py-2 focus:text-sm focus:font-medium focus:text-white">
        Skip to content
      </a>
      ...
      <aside className="flex w-56 shrink-0 flex-col border-r border-line bg-surface">
        ...
        `er-nav-item flex items-center gap-2.5 rounded-md px-3 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-ops-50 text-ops-700' : 'text-muted hover:bg-elevated hover:text-ink'}`}>
              <Icon size={16} />
              {label}
            </NavLink>)}
        </nav>
        <div className="space-y-2 border-t border-line-soft p-3">
          ...
          <button onClick={() => setLogoutOpen(true)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-muted hover:bg-elevated">
            <LogOut size={16} />
            Log out
          </button>
        </div>
      ...
      <main id="main-content" className="min-w-0 flex-1 overflow-y-auto">
        ...
        {!online && <div role="status" className="flex items-center justify-center gap-1.5 bg-amber-100 px-3 py-1.5 text-xs font-medium text-amber-800">
            <WifiOff size={13} />
      ...
      <ConfirmDialog open={logoutOpen} title="Log out?" message="You'll need to sign in again to access the ops dashboard." confirmLabel="Log out" danger loading={loggingOut} onConfirm={handleLogout} onCancel={() => setLogoutOpen(false)} />
    </div>;
}
