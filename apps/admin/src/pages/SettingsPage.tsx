...
type Thresholds = EventConfig['dispatch'];
type ThresholdKey = Exclude<keyof Thresholds, 'weights'>;

const THRESHOLD_FIELDS: Array<{ key: ThresholdKey; label: string; suffix?: string }> = [
  { key: 'batchHorizonMin', label: 'Batch horizon', suffix: 'min' },
  { key: 'maxDetourMin', label: 'Max detour', suffix: 'min' },
  { key: 'starvationThresholdMin', label: 'Starvation threshold', suffix: 'min' },
  { key: 'maxSharedGuestsPerTrip', label: 'Max shared guests / trip' },
  { key: 'clusterRadiusM', label: 'Cluster radius', suffix: 'm' },
  { key: 'clusterTimeWindowMin', label: 'Cluster time window', suffix: 'min' },
  { key: 'driverBreakAfterTrips', label: 'Break after N trips' },
  { key: 'driverBreakMinutes', label: 'Break duration', suffix: 'min' },
  { key: 'driverMaxContinuousMin', label: 'Max continuous drive', suffix: 'min' },
  { key: 'offerTimeoutSec', label: 'Offer timeout', suffix: 'sec' },
  { key: 'maxReassignAttempts', label: 'Max reassign attempts' }
];

export default function SettingsPage() {
  const qc = useQueryClient();
  const { push } = useToast();
  const [flags, setFlags] = useState<EventConfig['featureFlags'] | null>(null);
  ...
  const saveMutation = useMutation({
    ...
    onError: (e: any) => push({ kind: 'error', title: 'Save failed', description: e.message })
  });

  function updateFlag(key: keyof EventConfig['featureFlags'], value: boolean) {
