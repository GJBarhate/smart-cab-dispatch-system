...
function FeasibilityChip({
  ...
  const stats = useMemo(() => {
    ...
    const distances = idle.map(d => haversineKm(pickup, toLeafletTuple(d.currentLocation))).filter(km => km > 0 && km <= NEARBY_RADIUS_KM * 3).sort((a, b) => a - b);
    const nearby = distances.filter(km => km <= NEARBY_RADIUS_KM);
    return {
      nearbyCount: nearby.length,
      closestKm: distances[0]
    };
...
export default function ApprovalInbox() {
  ...
  return <div className="p-4 md:p-6">
      ...
      {requestsQ.isLoading ? <SkeletonRows rows={4} /> : requestsQ.isError ? <ErrorState message={requestsQ.error?.message} onRetry={() => requestsQ.refetch()} /> : !requestsQ.data?.length ? <EmptyState icon={InboxIcon} title="Inbox zero" description="No on-demand requests are waiting for approval right now." /> : <div className="er-stagger grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {requestsQ.data.map(r => <Card key={r.id}>
      ...
      <Modal open={!!declineTarget} onClose={() => setDeclineTarget(null)} title="Decline request" footer={<>
            <Button variant="secondary" onClick={() => setDeclineTarget(null)}>Cancel</Button>
            <Button variant="danger" loading={declineMutation.isPending} onClick={() => declineTarget && declineMutation.mutate({
        id: declineTarget.id,
        ...
        <div className="space-y-2">
          {DECLINE_REASONS.map(reason => <label key={reason} className="flex items-center gap-2 text-sm">
