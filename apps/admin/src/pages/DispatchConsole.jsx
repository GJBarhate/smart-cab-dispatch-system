...
const WEIGHT_META = [{
  ...
  key: 'priority',
  label: 'Priority (anti-starvation)',
  min: 0,
  max: 8
}, {
  key: 'idle',
...
const RAMP_LIGHT = [245, 243, 255]; // ops-50
const RAMP_DARK = [76, 29, 149]; // ops-800
...
export default function DispatchConsole() {
  ...
  const configWeightsMutation = useMutation({
    mutationFn: w => AdminApi.updateConfig({
      dispatch: {
        weights: w
      }
    }),
  ...
  const {
    ...
    return {
      minCost: Math.min(...feasible),
      maxCost: Math.max(...feasible)
    };
  }, [preview]);
  ...
  const selectedDetail = selected ? chosenMap.get(`${selected.i}:${selected.j}`) ?? {
    cost: preview?.costMatrix[selected.i]?.[selected.j] ?? 0,
    breakdown: null
  } : null;
      ...
      <div className="er-stagger grid grid-cols-1 gap-4 xl:grid-cols-4">
        ...
        <Card className="xl:col-span-1">
          ...
          <CardBody className="space-y-4">
            {configQ.isError ? <ErrorState message={configQ.error?.message} onRetry={() => configQ.refetch()} /> : !weights ? <Skeleton className="h-64 w-full" /> : WEIGHT_META.map(m => <Slider key={m.key} label={m.label} value={weights[m.key]} min={m.min} max={m.max} step={0.1} onChange={v => onWeightChange(m.key, v)} />)}
        ...
        <Card className="xl:col-span-3">
          <CardHeader title="Cost matrix heatmap" subtitle={preview ? `${preview.drivers.length} drivers × ${preview.demands.length} demand clusters` : undefined} action={<div className="flex items-center gap-3 text-[11px] text-muted">
                <span className="flex items-center gap-1"><span className="h-3 w-3 rounded-sm" style={{
              background: lerpColor(0.05)
            }} /> cheap</span>
                <span className="flex items-center gap-1"><span className="h-3 w-3 rounded-sm" style={{
              background: lerpColor(0.95)
            }} /> costly</span>
          ...
          <CardBody>
            {previewMutation.isPending && !preview ? <Skeleton className="h-72 w-full" /> : previewMutation.isError ? <ErrorState message={previewMutation.error?.message ?? 'Could not load the preview'} onRetry={() => previewMutation.mutate()} /> : !preview || preview.drivers.length === 0 || preview.demands.length === 0 ? <EmptyState icon={Sparkles} title="Nothing to preview" description="No eligible drivers or waiting demand right now — seed some queue entries or wait for guests to arrive." /> : <div className="flex flex-col gap-4 lg:flex-row">
                <div className="min-w-0 flex-1 overflow-auto rounded-lg border border-line-soft" style={{
              maxHeight: 460
            }}>
                  <table className="border-collapse text-[11px]">
                    <thead>
                      <tr>
                        ...
                        {preview.demands.map((d, j) => <th key={d.id} className="sticky top-0 z-10 min-w-[30px] bg-surface p-1 text-center font-medium text-muted" title={`Demand ${j + 1} — ${d.guestIds.length} guest(s), ${d.seats} seats`}>
                            R{j + 1}
                          </th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {preview.drivers.map((drv, i) => <tr key={drv.id}>
                      ...
                      return <td key={j} onClick={() => setSelected({
                        i,
                        j
                      })} className={`h-7 w-7 cursor-pointer text-center align-middle ${isSelectedCell ? 'ring-2 ring-ops-600 ring-inset' : ''} ${isChosen ? 'border-2 border-emerald-500' : 'border border-surface'}`} style={infeasible ? {
                        background: 'rgb(var(--c-elevated))',
                        backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 2px, rgb(var(--c-faint)) 2px, rgb(var(--c-faint)) 3px)'
                      } : {
                ...
                <div className="w-full shrink-0 rounded-lg border border-line-soft p-3 lg:w-64">
                  {!selected ? <div className="flex h-full flex-col items-center justify-center gap-1 py-8 text-center text-xs text-faint">
                      <Info size={18} />
                      ...
                      {chosenMap.has(`${selected.i}:${selected.j}`) ? <Badge tone="green">Hungarian-selected pair</Badge> : <Badge tone="gray">Not selected</Badge>}
                      {selectedDetail?.breakdown ? <div className="space-y-1 border-t border-line-soft pt-2">
                        ...
                        </div> : <p className="border-t border-line-soft pt-2 text-xs text-muted">
