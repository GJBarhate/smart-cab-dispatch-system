      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-4">
        <Card className="overflow-hidden lg:col-span-3">
          {dashboardQ.isLoading ? (
            ...
            <ErrorState message={(dashboardQ.error as any)?.message} onRetry={() => dashboardQ.refetch()} />
          ) : (
            ...
            {selected ? (
              ...
              <div className="space-y-2">
                ...
                {queueQ.isLoading ? (
                  ...
                  <ErrorState message={(queueQ.error as any)?.message} onRetry={() => queueQ.refetch()} />
                ) : (queueQ.data?.length ?? 0) === 0 ? (
                  <EmptyState icon={Users} title="No guests waiting" />
                ) : (
                  <ul className="space-y-1.5">
                    {(queueQ.data ?? []).slice(0, 10).map((q) => (
                      <li key={q.id} className="rounded-md border border-line-soft p-2 text-xs">
