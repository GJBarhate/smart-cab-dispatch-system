        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              description="Analytics are computed from completed trips. Run the dispatch engine or the simulation and these charts will populate."
            />
          </CardBody>
        </Card>
      ) : (
        <div className="er-stagger grid grid-cols-1 gap-4 xl:grid-cols-2">
          <ChartCard
            title="Wait-time distribution"
            subtitle="Guests per wait band, in minutes. The shape matters more than the mean — bursty arrivals hide a long tail behind a healthy average."
          >
            <BarChart data={data.waitDistribution} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={t.grid} />
              <XAxis dataKey="bucket" tick={{ fontSize: 11, fill: t.tick }} axisLine={{ stroke: t.axis }} tickLine={false} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: t.tick }} axisLine={false} tickLine={false} width={30} />
              <Tooltip contentStyle={t.tooltip} cursor={{ fill: t.grid, fillOpacity: 0.3 }} formatter={(v: number) => [v, 'Guests']} />
              <Bar dataKey="count" radius={[4, 4, 0, 0]} maxBarSize={56}>
                ...
                {data.waitDistribution.map((b, i) => (
                  <Cell key={b.bucket} fill={i >= 4 ? t.series.danger : i === 3 ? t.series.warning : t.series.primary} />
                ))}
              </Bar>
            </BarChart>
          </ChartCard>
          ...
          <ChartCard
            title="Throughput across the day"
            subtitle="Completed trips and guests carried, by hour — the utilisation curve, with the arrival peak visible."
          >
            <AreaChart data={data.tripsByHour} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={t.grid} />
              <XAxis dataKey="hour" tick={{ fontSize: 11, fill: t.tick }} axisLine={{ stroke: t.axis }} tickLine={false} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: t.tick }} axisLine={false} tickLine={false} width={30} />
          ...
          <ChartCard
            title="ETA accuracy"
            subtitle="Predicted minus actual, in minutes. Bars left of zero ran later than promised; the two green bands are within ±2 min."
          >
            <BarChart data={data.etaAccuracyDistribution} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={t.grid} />
              <XAxis dataKey="bucket" tick={{ fontSize: 11, fill: t.tick }} axisLine={{ stroke: t.axis }} tickLine={false} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: t.tick }} axisLine={false} tickLine={false} width={30} />
              <Tooltip contentStyle={t.tooltip} cursor={{ fill: t.grid, fillOpacity: 0.3 }} formatter={(v: number) => [v, 'Trips']} />
              <Bar dataKey="count" radius={[4, 4, 0, 0]} maxBarSize={56}>
                {data.etaAccuracyDistribution.map((b, i) => (
                  <Cell key={b.bucket} fill={i === 2 || i === 3 ? t.series.positive : t.series.primary} />
                ))}
              </Bar>
            </BarChart>
          </ChartCard>
          ...
          <ChartCard
            ...
            subtitle="A flat profile means the cost function's idle-time term is spreading work across the fleet rather than favouring the same few cars."
          >
            <BarChart data={data.tripsPerDriver} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={t.grid} />
              <XAxis
                dataKey="name"
          ...
          <ChartCard
            title="Assignments by strategy"
            subtitle="How completed trips were matched — batch Hungarian, greedy real-time, detour insertion, starvation sweep, or manual override."
          >
            <BarChart data={strategyData} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={t.grid} />
              <XAxis dataKey="strategy" tick={{ fontSize: 11, fill: t.tick }} axisLine={{ stroke: t.axis }} tickLine={false} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: t.tick }} axisLine={false} tickLine={false} width={30} />
              <Tooltip contentStyle={t.tooltip} cursor={{ fill: t.grid, fillOpacity: 0.3 }} formatter={(v: number) => [v, 'Trips']} />
          ...
          <Card>
            <CardHeader
              title="Sharing dividend"
              subtitle="Vehicle-minutes not driven because rides were combined, net of the detour each combination cost."
            />
            <CardBody>
              <div className="flex flex-col items-center justify-center gap-1 text-center" style={{ height: CHART_HEIGHT }}>
