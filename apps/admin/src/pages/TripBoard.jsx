...
const LANES = [{
  ...
  key: 'issues',
  label: 'Cancelled / Rejected / Unassignable',
  statuses: ['cancelled', 'rejected', 'unassignable'],
  tone: 'red'
}];
function TripCard({
  trip,
  ...
  return <div className="rounded-xl border border-line bg-surface p-3 shadow-sm">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        ...
        {canExplain && <Button size="sm" variant="ghost" title="Why this driver?" onClick={() => onExplain(trip)}>
...
export default function TripBoard() {
  ...
  useSocketEvent('trip:status', () => qc.invalidateQueries({
    queryKey: ['trips']
  }));
  const reassignMutation = useMutation({
    mutationFn: ({
      id,
      driverId
    }) => AdminApi.reassignTrip(id, driverId),
    onSuccess: () => {
      push({
        kind: 'success',
        title: 'Trip reassigned',
        description: 'Both the new driver and guests were notified.'
      });
  ...
  const cancelMutation = useMutation({
    mutationFn: ({
      id,
      reason
    }) => AdminApi.cancelTrip(id, reason),
    onSuccess: () => {
      push({
        kind: 'info',
        title: 'Trip cancelled',
        description: 'Guests were re-queued automatically.'
      });
  ...
  const lanes = useMemo(() => {
    ...
    return LANES.map(lane => ({
      ...lane,
      trips: trips.filter(t => lane.statuses.includes(t.status))
    }));
  }, [tripsQ.data]);
  ...
  return <div className="p-4 md:p-6">
      ...
      {tripsQ.isLoading ? <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-6">
          {Array.from({
        length: 6
      }).map((_, i) => <Skeleton key={i} className="h-64 w-full" />)}
        </div> : tripsQ.isError ? <ErrorState message={tripsQ.error?.message} onRetry={() => tripsQ.refetch()} /> : (tripsQ.data?.length ?? 0) === 0 ? <EmptyState icon={Kanban} title="No trips yet" description="Trips created by the dispatch engine or manual overrides will show up here." /> : <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-6">
          {lanes.map(lane => <div key={lane.key} className="min-w-0">
              <div className="mb-2 flex items-center justify-between">
                <Badge tone={lane.tone}>{lane.label}</Badge>
                <span className="text-xs text-faint">{lane.trips.length}</span>
              </div>
              <div className="space-y-2">
                {lane.trips.length === 0 ? <p className="rounded-lg border border-dashed border-line p-3 text-center text-[11px] text-faint">Empty</p> : lane.trips.map(t => <TripCard key={t.id} trip={t} onReassign={setReassignTarget} onCancel={setCancelTarget} onExplain={setExplainTarget} />)}
              </div>
            </div>)}
        </div>}

      <Modal open={!!reassignTarget} onClose={() => {
      setReassignTarget(null);
      setChosenDriverId('');
    }} title={`Reassign ${reassignTarget?.code ?? ''}`} footer={<>
            <Button variant="secondary" onClick={() => setReassignTarget(null)}>Cancel</Button>
            <Button disabled={!chosenDriverId} loading={reassignMutation.isPending} onClick={() => reassignTarget && reassignMutation.mutate({
        ...
        {driversQ.isLoading ? <Skeleton className="h-24 w-full" /> : availableDrivers.length === 0 ? <EmptyState title="No idle drivers available" /> : <div className="max-h-64 space-y-1.5 overflow-y-auto">
            {availableDrivers.map(d => <label key={d.id} className="flex items-center gap-2 rounded-md border border-line-soft p-2 text-sm">
      ...
      <Modal open={!!cancelTarget} onClose={() => setCancelTarget(null)} title={`Cancel ${cancelTarget?.code ?? ''}`} footer={<>
            <Button variant="secondary" onClick={() => setCancelTarget(null)}>Back</Button>
            <Button variant="danger" loading={cancelMutation.isPending} onClick={() => cancelTarget && cancelMutation.mutate({
        id: cancelTarget.id,
      ...
      <ExplainAssignment tripId={explainTarget?.id ?? null} tripCode={explainTarget?.code} onClose={() => setExplainTarget(null)} />
    </div>;
}
