...
async function fetchCurrentTripOrNull() {
  try {
...
export default function DriverHome() {
  ...
  const summaryQ = useQuery({
    ...
    queryFn: DriverApi.summary,
    refetchInterval: 30_000
  });
  const tripQ = useQuery({
    queryKey: ['driver', 'currentTrip'],
    queryFn: fetchCurrentTripOrNull,
  ...
  useEffect(() => {
    if (socket && trip?.id) socket.emit('trip:subscribe', {
      tripId: trip.id
    });
  }, [socket, trip?.id]);
  useSocketEvent('trip:offered', () => qc.invalidateQueries({
    queryKey: ['driver', 'currentTrip']
  }));
  ...
  const statusMutation = useMutation({
    mutationFn: action => DriverApi.setStatus(action),
    onSuccess: (_d, action) => {
      qc.invalidateQueries({
        queryKey: ['driver', 'me']
      });
      if (action === 'request_break') push({
        kind: 'success',
        title: 'Break started',
        description: '20 minutes off the clock.'
      });
    },
    onError: e => push({
      kind: 'error',
  ...
  const acceptMutation = useMutation({
    ...
    onSuccess: () => {
      push({
        kind: 'success',
        title: 'Trip accepted',
        description: "You're en route to pickup."
      });
      qc.invalidateQueries({
        queryKey: ['driver', 'currentTrip']
      });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      push({
        kind: 'info',
        title: 'Trip declined',
        description: 'It has been re-queued for another driver.'
      });
      qc.invalidateQueries({
        queryKey: ['driver', 'currentTrip']
      });
  ...
  const dropMutation = useMutation({
    ...
    onSuccess: updated => {
      ...
      if (updated.status === 'completed') push({
        kind: 'success',
        title: 'Trip completed',
        description: 'Great work — waiting for your next assignment.'
      });
    },
    onError: e => push({
      kind: 'error',
      title: 'Could not complete drop',
      description: e.message
    })
  });
  async function handleLogout() {
    ...
    } catch {/* client discard proceeds regardless */}
    clear();
  }
  if (meQ.isLoading) {
    return <div className="flex min-h-screen flex-col gap-4 bg-canvas p-4">
  ...
  return <div className="min-h-screen bg-canvas">
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-emerald-100 bg-surface px-4 py-3 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <Toggle checked={isOnline} disabled={!!trip && trip.status !== 'completed'} onChange={v => statusMutation.mutate(v ? 'online' : 'offline')} accent="emerald" />
          <ThemeToggle compact />
      ...
      <main className="mx-auto flex max-w-md flex-col gap-4 p-4">
        {tripQ.isLoading ? <Skeleton className="h-40 w-full" /> : !trip ? <div className="rounded-2xl border border-dashed border-emerald-200 bg-surface p-8 text-center">
          ...
          </div> : trip.status === 'pending_driver' ? <OfferCard trip={trip} secondsLeft={secondsLeft} onAccept={() => acceptMutation.mutate()} onReject={() => setRejectOpen(true)} accepting={acceptMutation.isPending} /> : <>
            ...
            {(trip.status === 'accepted' || trip.status === 'en_route_pickup') && <Button size="lg" variant="success" className="w-full py-5 text-base" onClick={() => arrivedMutation.mutate()} loading={arrivedMutation.isPending}>
                <Navigation2 size={18} /> ARRIVED AT PICKUP
              </Button>}
            {trip.status === 'at_pickup' && <Button size="lg" variant="success" className="w-full py-5 text-base" onClick={() => boardMutation.mutate()} loading={boardMutation.isPending}>
                <Users size={18} /> BOARD GUESTS ({trip.guests.length})
              </Button>}
            {trip.status === 'boarded' && nextDropStop && <Button size="lg" variant="success" className="w-full py-5 text-base" onClick={() => dropMutation.mutate()} loading={dropMutation.isPending}>
                <PackageCheck size={18} /> DROP AT {nextDropStop.label || `STOP ${nextDropStop.seq}`}
              </Button>}
            {['completed', 'cancelled', 'rejected', 'unassignable'].includes(trip.status) && <div className="flex items-center gap-2 rounded-xl bg-surface p-4 text-sm text-muted shadow-sm">
        ...
        <div className="rounded-xl border border-line-soft bg-surface p-4 shadow-sm">
          <p className="text-sm text-muted">
            ...
            {' · '}
            {summaryQ.data?.onBreakUntil ? <span className="font-semibold text-amber-600">On break</span> : <span>Since last break: <span className="font-semibold text-ink">{summaryQ.data?.tripsSinceBreak ?? 0}</span> trips</span>}
          </p>
          {!trip && isOnline && !summaryQ.data?.onBreakUntil && <Button variant="secondary" size="sm" className="mt-3 w-full" onClick={() => statusMutation.mutate('request_break')} loading={statusMutation.isPending}>
              <Coffee size={12} /> Request break
            </Button>}
        </div>
      </main>

      <Modal open={rejectOpen} onClose={() => setRejectOpen(false)} title="Why are you declining?" footer={<>
        ...
        <div className="space-y-2">
          {REJECT_REASONS.map(r => <label key={r} className="flex items-center gap-2 rounded-md border border-line-soft p-3 text-sm">
      ...
      <ConfirmDialog open={logoutOpen} title="Log out?" message={trip && !['completed', 'cancelled', 'rejected', 'unassignable'].includes(trip.status) ? "You have an active trip in progress. Logging out won't cancel it, but you'll stop receiving updates until you sign back in." : "You'll need to sign in again to receive trip offers."} confirmLabel="Log out" danger loading={loggingOut} onConfirm={handleLogout} onCancel={() => setLogoutOpen(false)} />
    </div>;
}
function OfferCard({
  trip,
  ...
  return <div className="relative overflow-hidden rounded-2xl border-2 border-emerald-300 bg-surface p-5 shadow-md">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="relative flex h-11 w-11 items-center justify-center">
          <svg viewBox="0 0 40 40" className="h-11 w-11 -rotate-90">
            ...
            <circle cx="20" cy="20" r="17" fill="none" stroke="#059669" strokeWidth="4" strokeDasharray={2 * Math.PI * 17} strokeDashoffset={2 * Math.PI * 17 * (1 - pct / 100)} strokeLinecap="round" />
          </svg>
...
function TripStatusCard({
  ...
  const labels = {
    ...
    at_pickup: 'Arrived — board your guests',
    boarded: 'Guests aboard — heading to drop'
  };
  const guestCount = trip.guests.length;
  return <div className="rounded-xl bg-surface p-4 shadow-sm">
