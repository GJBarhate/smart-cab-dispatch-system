...
export default function GuestManagement() {
  ...
  const [modeDraft, setModeDraft] = useState('flight');
  const [pendingImport, setPendingImport] = useState(null);
  const guestsQ = useQuery({
    queryKey: ['guests'],
    queryFn: () => AdminApi.guests(),
  ...
  const updateArrivalMutation = useMutation({
    ...
    onSuccess: () => {
      push({
        ...
        description: 'Any waiting queue entry was re-evaluated.'
      });
      setEditTarget(null);
      qc.invalidateQueries({
        queryKey: ['guests']
  ...
  return <div className="p-4 md:p-6">
      ...
      <Card className="overflow-hidden">
        {guestsQ.isLoading ? <div className="p-4"><SkeletonRows rows={6} /></div> : guestsQ.isError ? <div className="p-4"><ErrorState message={guestsQ.error?.message} onRetry={() => guestsQ.refetch()} /></div> : !guestsQ.data?.length ? <div className="p-4"><EmptyState icon={Users} title="No guests yet" description="Add a walk-in or import a CSV to get started." /></div> : <div className="overflow-x-auto">
            <table className="er-table w-full min-w-[800px] text-sm">
              ...
              <tbody className="divide-y divide-line-soft">
                {guestsQ.data.map(g => <tr key={g.id}>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add walk-in guest">
        <form onSubmit={submitCreate} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Booking ref"><input required value={form.bookingRef} onChange={e => setForm({
              ...form,
            ...
            <Field label="Group size"><input type="number" min={1} value={form.groupSize} onChange={e => setForm({
              ...form,
              groupSize: Number(e.target.value)
            })} className="input" /></Field>
            <Field label="Luggage"><input type="number" min={0} value={form.luggageCount} onChange={e => setForm({
              ...form,
              luggageCount: Number(e.target.value)
            })} className="input" /></Field>
          </div>
          <label className="flex items-center gap-2 text-sm text-muted">
            <input type="checkbox" checked={form.isVip} onChange={e => setForm({
            ...form,
            isVip: e.target.checked
          })} />
            VIP priority tier
          </label>
          <Field label="Special needs"><input value={form.specialNeeds} onChange={e => setForm({
            ...form,
            specialNeeds: e.target.value
          })} className="input" placeholder="e.g. wheelchair" /></Field>
          <Button type="submit" className="w-full" loading={createMutation.isPending}>Add guest</Button>
        </form>
      </Modal>

      <Modal open={!!editTarget} onClose={() => setEditTarget(null)} title={`Edit arrival — ${editTarget?.name ?? ''}`} footer={<>
            <Button variant="secondary" onClick={() => setEditTarget(null)}>Cancel</Button>
            <Button loading={updateArrivalMutation.isPending} disabled={!arrivalDraft} onClick={() => editTarget && updateArrivalMutation.mutate({
        id: editTarget.id,
        scheduledAt: arrivalDraft,
        mode: modeDraft
      })}>
        ...
        <div className="space-y-3">
          <Field label="Mode">
            <select value={modeDraft} onChange={e => setModeDraft(e.target.value)} className="input">
      ...
      <ConfirmDialog open={!!pendingImport} title="Import guests from CSV?" message={pendingImport ? `"${pendingImport.name}" will be parsed and any valid rows added as new guests. This can't be undone in bulk.` : ''} confirmLabel="Import" loading={importMutation.isPending} onConfirm={() => pendingImport && importMutation.mutate(pendingImport)} onCancel={() => setPendingImport(null)} />
    </div>;
}
function Field({
  label,
