...
const BACKGROUNDS = ['canvas', 'surface', 'elevated'];
const FOREGROUNDS = ['ink', 'muted', 'faint'];
...
describe.each([['light', ':root'], ['dark', '.dark']])('%s theme contrast', (_name, selector) => {
  ...
  it.each(FOREGROUNDS.flatMap(fg => BACKGROUNDS.map(bg => [fg, bg])))('%s on %s meets WCAG AA for normal text', (fg, bg) => {
    const ratio = contrast(t[fg], t[bg]);
    expect(ratio, `--c-${fg} on --c-${bg} is ${ratio.toFixed(2)}:1, below the ${AA_NORMAL_TEXT}:1 AA threshold`).toBeGreaterThanOrEqual(AA_NORMAL_TEXT);
  });
