...
export function useCountdownSeconds(targetIso: string | null | undefined): number | null {
  const [secondsLeft, setSecondsLeft] = useState<number | null>(() => (targetIso ? Math.round((new Date(targetIso).getTime() - Date.now()) / 1000) : null));
...
export function useElapsedSeconds(sinceIso: string | null | undefined): number | null {
  const [elapsed, setElapsed] = useState<number | null>(() => (sinceIso ? Math.max(0, Math.round((Date.now() - new Date(sinceIso).getTime()) / 1000)) : null));
