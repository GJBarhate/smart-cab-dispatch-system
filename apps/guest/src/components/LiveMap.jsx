...
function useAnimatedPosition(target, durationMs = 4000) {
...
function AutoFollow({
  ...
  useEffect(() => {
    if (!firstFit.current && bounds.length > 0) {
      ...
      if (bounds.length === 1) {
        ...
        map.fitBounds(bounds.map(b => [b.lat, b.lng]), {
          padding: [40, 40]
        });
      }
    }
  ...
  useEffect(() => {
    if (autoFollow && driverPos) {
      map.panTo([driverPos.lat, driverPos.lng], {
        animate: true,
        duration: 0.8
      });
    }
...
function RecenterFab({
  ...
  return <button onClick={onClick} aria-label="Recenter map" className="absolute bottom-4 right-4 z-[1000] flex h-12 w-12 items-center justify-center rounded-full bg-surface shadow-lg ring-1 ring-line active:scale-95">
...
export function LiveMap({
  ...
  const animatedDriverPos = useAnimatedPosition(driverPosition ? {
  ...
  return <div className={`relative overflow-hidden ${className}`}>
      <MapContainer key={mapId} center={center} zoom={14} scrollWheelZoom className="h-full w-full" attributionControl>
        ...
        {polyline.length > 1 && <Polyline positions={polyline} pathOptions={{
        color: '#2563eb',
        weight: 4,
        opacity: 0.8
      }} />}
        ...
        {animatedDriverPos && <Marker position={[animatedDriverPos.lat, animatedDriverPos.lng]} icon={carIcon(driverPosition?.heading ?? 0)} />}
