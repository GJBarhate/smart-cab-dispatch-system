...
export async function makeDriver(name = 'Test Driver') {
  ...
  const user = await User.create({ name, phone, role: 'driver', passwordHash: 'x', isActive: true });
  const driver = await Driver.create({
    userId: user._id,
    name,
    phone,
    vehicle: { number: `KA-${n}`, model: 'Sedan', colour: 'White', type: 'sedan' },
    capacity: { seats: 4, luggage: 3 },
    status: 'idle',
    currentLocation: toGeoPoint({ lat: 18.55, lng: 73.85 }),
    predictedFreeAt: new Date(),
    predictedFreeLocation: toGeoPoint({ lat: 18.55, lng: 73.85 }),
    isActive: true
  });
...
export async function makeAdmin(name = 'Test Admin') {
  const suffix = unique();
  const user = await User.create({ name, email: `admin${suffix}@test.com`, role: 'admin', passwordHash: 'x', isActive: true });
