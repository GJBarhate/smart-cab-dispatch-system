...
async function makeWaitingEntry(deadlineMinFromNow) {
  const {
    guest
  } = await makeGuest();
...
describe('expired demand retirement', () => {
  it('retires an entry whose deadline is long past, instead of retrying it forever', async () => {
    ...
    const {
      entry
    } = await makeWaitingEntry(-360); // 6 hours past, as seen in production
