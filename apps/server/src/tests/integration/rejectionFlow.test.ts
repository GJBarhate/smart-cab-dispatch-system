...
describe('driver rejection flow', () => {
  it('re-queues the guest at boosted priority and blacklists the rejecting driver for that entry', async () => {
    ...
    await Driver.updateOne({ _id: backup.driver._id }, { $set: { status: 'idle' } });
    ...
    const rejectRes = await request(app)
      .post(`/api/driver/trip/${trip!._id}/reject`)
      .set('Authorization', `Bearer ${rejectingToken}`)
      .send({ reason: 'vehicle issue' });
    expect(rejectRes.status).toBe(200);
    expect(rejectRes.body.data.status).toBe('rejected');

    const entry = await QueueEntry.findOne({ guestIds: guest._id });
    ...
    const newTrip = await Trip.findOne({ status: { $ne: 'rejected' } });
