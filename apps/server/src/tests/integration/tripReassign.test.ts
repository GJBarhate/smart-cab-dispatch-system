...
async function makeAssignedTrip(driverId: unknown, guestId: unknown) {
  const trip = await Trip.create({
    code: 'T-REASSIGN',
    type: 'ON_DEMAND',
    status: 'en_route_pickup',
    driverId,
    vehicleSnapshot: { number: 'KA-01', model: 'Sedan', seats: 4, luggage: 3 },
    stops: [],
    capacityUsed: { seats: 1, luggage: 1 },
    guests: [{ guestId, name: 'Test Guest', seats: 1, luggage: 1 }],
    assignmentMeta: {
  ...
  await Driver.updateOne({ _id: driverId }, { $set: { currentTripId: trip._id, status: 'assigned' } });
  return trip;
}
...
describe('POST /api/admin/trips/:id/reassign', () => {
  it('succeeds and preserves the engine cost breakdown', async () => {
    ...
    const res = await request(app)
      .post(`/api/admin/trips/${trip._id}/reassign`)
      .set('Authorization', `Bearer ${admin.token}`)
      .send({ driverId: to.driver._id.toString() });
  ...
  it('409s rather than stranding the trip when the target driver is busy', async () => {
    ...
    await Driver.updateOne({ _id: to.driver._id }, { $set: { currentTripId: trip._id } });
  ...
  it('404s for a trip that does not exist', async () => {
    ...
    const res = await request(app)
      .post('/api/admin/trips/aaaaaaaaaaaaaaaaaaaaaaaa/reassign')
      .set('Authorization', `Bearer ${admin.token}`)
      .send({ driverId: to.driver._id.toString() });
