...
async function makeTrip(driverId, guestId) {
  return Trip.create({
...
describe('Trip state machine', () => {
  it('rejects every invalid transition with a 409-style ConflictError', async () => {
    ...
    await expect(TripService.transition(trip._id.toString(), 'boarded', 'test')).rejects.toMatchObject({
      status: 409
    });
    await expect(TripService.transition(trip._id.toString(), 'completed', 'test')).rejects.toMatchObject({
      status: 409
    });
    await expect(TripService.transition(trip._id.toString(), 'at_pickup', 'test')).rejects.toMatchObject({
      status: 409
    });
  });
  it('rejects transitioning out of a terminal state', async () => {
    ...
    await expect(TripService.transition(trip._id.toString(), 'accepted', 'test')).rejects.toMatchObject({
      status: 409
    });
  });
  ...
  it('refuses to yank a boarded guest into unassignable', async () => {
    ...
    await TripService.transition(trip._id.toString(), 'at_pickup', 'test');
    await TripService.transition(trip._id.toString(), 'boarded', 'test');
    await expect(TripService.transition(trip._id.toString(), 'unassignable', 'reoptimizer')).rejects.toMatchObject({
      status: 409
    });
  });
});
