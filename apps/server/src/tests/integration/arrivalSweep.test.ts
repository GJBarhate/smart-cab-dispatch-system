...
async function makeArrivingGuest(minutesFromNow: number) {
  const { guest } = await makeGuest();
  const { airport, hotel } = await makeLocations();
...
describe('arrival sweep', () => {
  ...
  it('leaves arrivals beyond the lookahead window alone', async () => {
    ...
    const { guest } = await makeArrivingGuest(180);
  ...
  it('skips a guest who already holds an open queue entry from another source', async () => {
    ...
    await QueueEntry.create({
      ...
      dropoff: { locationId: hotel._id, coordinates: hotel.coordinates, label: hotel.name },
      earliestAt: new Date(),
  ...
  it('skips a guest already on a trip', async () => {
    await makeEventConfig();
    const { guest } = await makeArrivingGuest(10);
    const trip = await Trip.create({ code: 'T-9001', type: 'ARRIVAL_PICKUP', status: 'accepted' });
    await Guest.updateOne({ _id: guest._id }, { $set: { currentTripId: trip._id } });
  ...
  it('skips guests missing a pickup location or accommodation', async () => {
    ...
    await Guest.updateOne(
      { _id: guest._id },
      { $set: { status: 'registered', 'arrival.scheduledAt': new Date(Date.now() + 5 * 60_000) } }
    );
