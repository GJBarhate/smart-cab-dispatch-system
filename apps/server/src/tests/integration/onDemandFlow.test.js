...
describe('on-demand ride flow', () => {
  it('guest request -> admin approve -> auto-match -> driver lifecycle -> completed', async () => {
    ...
    const approveRes = await request(app).post(`/api/admin/requests/${reqRes.body.data.id}/approve`).set('Authorization', `Bearer ${adminToken}`);
    ...
    const boardRes = await request(app).post(`/api/driver/trip/${trip._id}/board`).set('Authorization', `Bearer ${driverToken}`).send({
      guestIds: [guest._id.toString()]
    });
    expect(boardRes.status).toBe(200);
    const dropRes = await request(app).post(`/api/driver/trip/${trip._id}/drop`).set('Authorization', `Bearer ${driverToken}`).send({
      guestIds: [guest._id.toString()],
      stopSeq: 1
    });
