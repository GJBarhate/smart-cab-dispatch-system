...
describe('Socket.IO realtime layer', () => {
  ...
  it('an admin socket receives driver:position broadcasts', async () => {
    ...
    await Promise.all([
      ...
      new Promise<void>((resolve) => driverClient.on('connect', () => resolve()))
    ]);

    const positionPromise = new Promise<any>((resolve) => adminClient.on('driver:position', resolve));
    driverClient.emit('driver:location', { lat: 18.6, lng: 73.9, heading: 90, speedKmph: 20 });
  ...
  it("a driver only joins their own trip room, never a global broadcast room", async () => {
    ...
    await Driver.updateOne({ _id: a.driver._id }, { $set: { currentTripId: null } });
    clientA.emit('driver:location', { lat: 18.61, lng: 73.91, heading: 0, speedKmph: 10 });
