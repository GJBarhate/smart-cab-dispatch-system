...
import { Clusterer, type ClusterableEntry, type ClustererConfig } from '../../services/dispatch/Clusterer';

const NOW = new Date('2026-08-10T09:00:00.000Z');

const cfg: ClustererConfig = {
...
function makeEntry(overrides: Partial<ClusterableEntry>): ClusterableEntry {
  return {
    id: 'e',
    type: 'ARRIVAL_PICKUP',
    guestIds: ['g'],
...
describe('Clusterer.build', () => {
  it('merges same-pickup/same-drop entries within the time window', () => {
    const entries = [
      makeEntry({ id: 'e1', guestIds: ['g1'] }),
      makeEntry({ id: 'e2', guestIds: ['g2'], earliestAt: new Date('2026-08-10T09:05:00.000Z') })
    ];
  ...
  it('does not merge pickups further apart than the cluster radius', () => {
    const entries = [
      makeEntry({ id: 'e1', guestIds: ['g1'], pickupLocationId: null, pickup: { lat: 18.58, lng: 73.9 } }),
      // ~5.5km away — well outside a 400m radius
      makeEntry({ id: 'e2', guestIds: ['g2'], pickupLocationId: null, pickup: { lat: 18.63, lng: 73.9 } })
    ];
    const clusters = Clusterer.build(entries, cfg, NOW);
  ...
  it('does not merge entries outside the time bucket window even at the same pickup', () => {
    const entries = [
      makeEntry({ id: 'e1', guestIds: ['g1'], earliestAt: new Date('2026-08-10T09:00:00.000Z') }),
      makeEntry({ id: 'e2', guestIds: ['g2'], earliestAt: new Date('2026-08-10T09:45:00.000Z') })
    ];
    const clusters = Clusterer.build(entries, cfg, NOW);
  ...
  it('respects maxSharedGuestsPerTrip', () => {
    const tightCfg: ClustererConfig = { ...cfg, maxSharedGuestsPerTrip: 2 };
    const entries = [
      makeEntry({ id: 'e1', guestIds: ['g1'] }),
      makeEntry({ id: 'e2', guestIds: ['g2'] }),
      makeEntry({ id: 'e3', guestIds: ['g3'] })
    ];
