...
import type { Phase } from '../../services/EventPhaseService';

const PHASES: Phase[] = [
  { key: 'ARRIVAL', startAt: new Date('2026-08-10T00:00:00Z'), endAt: new Date('2026-08-11T18:00:00Z'), defaultTripType: 'ARRIVAL_PICKUP' },
  { key: 'EVENT_DAY', startAt: new Date('2026-08-11T18:00:00Z'), endAt: new Date('2026-08-13T12:00:00Z'), defaultTripType: 'TO_VENUE' },
  { key: 'DEPARTURE', startAt: new Date('2026-08-13T12:00:00Z'), endAt: new Date('2026-08-14T23:59:00Z'), defaultTripType: 'DEPARTURE_DROP' }
];
...
describe('EventPhaseService', () => {
  ...
  it('maps the active phase to its default trip type', () => {
    expect(defaultTripType({ phases: PHASES } as never, new Date('2026-08-12T09:00:00Z'))).toBe('TO_VENUE');
    expect(defaultTripType({ phases: PHASES } as never, new Date('2026-08-13T18:00:00Z'))).toBe('DEPARTURE_DROP');
  });
  ...
  it('falls back to ARRIVAL_PICKUP rather than throwing when no phase applies', () => {
    ...
    expect(defaultTripType({ phases: PHASES } as never, new Date('2027-01-01T00:00:00Z'))).toBe('ARRIVAL_PICKUP');
    expect(defaultTripType(null, new Date())).toBe('ARRIVAL_PICKUP');
    expect(defaultTripType({ phases: [] } as never, new Date())).toBe('ARRIVAL_PICKUP');
  });

  it('rejects a phase whose defaultTripType is not a known trip type', () => {
    const bogus: Phase[] = [
      { key: 'X', startAt: new Date('2026-08-10T00:00:00Z'), endAt: new Date('2026-08-20T00:00:00Z'), defaultTripType: 'NOT_A_TYPE' }
    ];
    expect(defaultTripType({ phases: bogus } as never, new Date('2026-08-11T00:00:00Z'))).toBe('ARRIVAL_PICKUP');
  });
});
