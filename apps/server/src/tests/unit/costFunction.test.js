...
describe('priorityScore — anti-starvation term', () => {
  it('scores a longer-waiting guest strictly higher than a closer, newer one', () => {
    const longWait = makeDemand({
      waitedMinutes: 30
    });
    const shortWait = makeDemand({
      waitedMinutes: 5
    });
  ...
  it('is superlinear: a 40min wait scores ~8x a 10min wait, holding everything else equal', () => {
    ...
    const long = makeDemand({
      priorityTier: 0,
      waitedMinutes: 40
    });
    const short = makeDemand({
      priorityTier: 0,
      waitedMinutes: 10
    });
  ...
  it('ranks a VIP above a standard guest at equal wait', () => {
    const vip = makeDemand({
      priorityTier: 3
    });
    const standard = makeDemand({
      priorityTier: 1
    });
...
describe('CostFunction.score', () => {
  const baseCtx = {
    now: NOW,
    pickupEtaMin: 10,
    arrivalAt: new Date('2026-08-10T09:20:00.000Z')
  };
  it('prefers an idle driver over an equidistant busy-soon driver', () => {
    const idleDriver = makeDriver({
      idleMinutes: 40
    });
    const busyDriver = makeDriver({
      idleMinutes: 2
    });
  ...
  it('total cost is always >= 0 after the offset, even under extreme inputs', () => {
    ...
    const {
      total
    } = CostFunction.score(driver, demand, baseCtx, cfg);
  ...
  it('adds a detour penalty only when detourAddedMin is supplied', () => {
    ...
    const detoured = CostFunction.score(driver, demand, {
      ...baseCtx,
      detourAddedMin: 6
    }, cfg).total;
