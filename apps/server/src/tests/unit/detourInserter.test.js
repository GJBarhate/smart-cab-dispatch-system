...
import { breaksAnyExistingDeadline, capacityHoldsThroughout, createEvaluationBudget, insertAt } from '../../services/dispatch/DetourInserter';
function stop(kind, seats, luggage = seats) {
  return {
    kind,
    guestIds: [],
    locationId: null,
...
describe('capacityHoldsThroughout', () => {
  it('accepts a sequence that never exceeds capacity', () => {
    ...
    expect(capacityHoldsThroughout(seq, {
      seats: 4,
      luggage: 4
    })).toBe(true);
  });
