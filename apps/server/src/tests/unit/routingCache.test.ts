...
describe('DistanceCacheService', () => {
  it('geohash bucketing collapses nearby coordinates to one key', () => {
    const a = { lat: 18.5590, lng: 73.7997 };
    const bNearby = { lat: 18.5591, lng: 73.7998 }; // ~15m away, well inside a ~153m geohash-7 bucket
    const dest = { lat: 18.5286, lng: 73.8745 };
  ...
  it('serves a second identical lookup from L1 with zero DB/network round-trips', async () => {
    const from = { lat: 18.5590, lng: 73.7997 };
    const to = { lat: 18.5286, lng: 73.8745 };

    expect(await DistanceCacheService.get(from, to)).toBeNull();
    await DistanceCacheService.set(from, to, { distanceMeters: 5000, durationSeconds: 600, provider: 'osrm' });

    const findOneSpy = vi.spyOn(mongoose.models.DistanceCache, 'findOne');
    const hit = await DistanceCacheService.get(from, to);

    expect(hit).toEqual({ distanceMeters: 5000, durationSeconds: 600, provider: 'osrm' });
...
describe('RoutingService fallback chain', () => {
  ...
  it('opens the OSRM breaker after repeated failures and serves haversine estimates without throwing', async () => {
    for (let i = 0; i < 4; i++) {
      ...
      const result = await RoutingService.eta({ lat: from.lat + i * 0.2, lng: from.lng }, to);
    ...
    const finalResult = await RoutingService.eta({ lat: from.lat + 1, lng: from.lng }, to);
