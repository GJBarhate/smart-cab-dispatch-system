...
import { GroupSplitter, type SplitBinConfig, type SplittableMember } from '../../services/dispatch/GroupSplitter';

const cfg: SplitBinConfig = { maxSeats: 7, maxLuggage: 7 };

describe('GroupSplitter', () => {
  it('splits a 14-person group with a 7-seat max into 2 chunks sharing a groupSplitId', () => {
    const guestIds = Array.from({ length: 14 }, (_, i) => `guest-${i}`);
    const members: SplittableMember[] = [{ id: 'booking-14', guestIds, seats: 14, luggage: 14 }];
  ...
  it('keeps families / same-booking guests together across chunks', () => {
    const members: SplittableMember[] = [
      { id: 'family-a', guestIds: ['a1', 'a2', 'a3'], seats: 3, luggage: 3 },
      { id: 'family-b', guestIds: ['b1', 'b2'], seats: 2, luggage: 2 },
      { id: 'family-c', guestIds: ['c1', 'c2', 'c3', 'c4'], seats: 4, luggage: 4 }
    ];
  ...
  it('does not require a split when total seats fit within one vehicle', () => {
    const members: SplittableMember[] = [{ id: 'small', guestIds: ['x'], seats: 3, luggage: 2 }];
