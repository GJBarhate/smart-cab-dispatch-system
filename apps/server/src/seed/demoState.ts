...
import { Driver, Guest, Location, Trip, RideRequest, QueueEntry, Alert, AuditLog, User } from '../models';
...
type GuestDoc = InstanceType<typeof Guest>;
...
const jitter = (base: number, spread: number): number => base + (rand() - 0.5) * 2 * spread;
 ...
 */
interface BackdatableModel {
  collection: { updateOne(filter: object, update: object): Promise<unknown> };
}

async function backdate(model: BackdatableModel, id: unknown, at: Date): Promise<void> {
  await model.collection.updateOne({ _id: id }, { $set: { createdAt: at, updatedAt: at } });
}

function driveMinutes(a: LatLng, b: LatLng): number {
  return ((haversineKm(a, b) * ROAD_FACTOR) / AVG_SPEED_KMPH) * 60;
}

/** A point `fraction` of the way from a to b — used to park in-flight drivers mid-route. */
function between(a: LatLng, b: LatLng, fraction: number): LatLng {
  return { lat: a.lat + (b.lat - a.lat) * fraction, lng: a.lng + (b.lng - a.lng) * fraction };
}

async function tripCode(): Promise<string> {
  return `T-${String(await nextSequence('trip')).padStart(4, '0')}`;
}
...
function buildStops(specs: StopSpec[]) {
 ...
 */
function costBreakdown(etaMin: number, opts: { priorityTier: number; idleMin: number; capacityWaste: number; detourMin?: number }) {
...
const STRATEGIES = ['batch_hungarian', 'batch_hungarian', 'batch_hungarian', 'greedy_realtime', 'greedy_realtime', 'detour_insert', 'starvation_sweep', 'manual_override'] as const;

// ---------------------------------------------------------------------------

export interface DemoStateReport {
...
export async function seedDemoState(): Promise<DemoStateReport> {
  const now = new Date();
  ...
  const hotels = [
    byName.get('Hotel Aurora, Koregaon Park')!,
    byName.get('Grand Meridian, Hinjewadi')!,
    byName.get('Riverside Suites, Kalyani Nagar')!
  ];
  ...
  const guests = await Guest.find({}).sort({ bookingRef: 1 });
  ...
  const report: DemoStateReport = { completedTrips: 0, liveTrips: 0, pendingRequests: 0, queueEntries: 0, alerts: 0 };
  ...
  const completedSpecs: Array<{ minutesAgo: number; guests: GuestDoc[]; from: LocationDoc; to: LocationDoc; type: string }> = [];
  ...
  historyGuests.forEach((g, i) => {
    ...
    completedSpecs.push({ minutesAgo: peakOffsets[i % peakOffsets.length], guests: [g], from, to, type: 'ARRIVAL_PICKUP' });
  });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    completedSpecs.push({ minutesAgo: laterOffsets[i], guests: [a, b], from: hotels[i % hotels.length], to: venue, type: 'TO_VENUE' });
  }

  // Venue -> hotel returns later in the day.
  for (let i = 0; i < 5; i++) {
    const g = historyGuests[i + 4];
    completedSpecs.push({ minutesAgo: laterOffsets[i + 3], guests: [g], from: venue, to: hotels[i % hotels.length], type: 'FROM_VENUE' });
  }
  ...
  for (let i = 0; i < completedSpecs.length; i++) {
    ...
    const fromLL = toLatLng(spec.from.coordinates as never);
    const toLL = toLatLng(spec.to.coordinates as never);
    const legMin = driveMinutes(fromLL, toLL);
    const pickupAt = addMinutes(startedAt, Math.round(jitter(9, 4)));
    const completedAt = addMinutes(pickupAt, Math.round(legMin));
    ...
    const completedTrip = await Trip.create({
      ...
      stops: buildStops([
        { kind: 'pickup', location: spec.from, guestIds: spec.guests.map((g) => g._id), at: pickupAt, done: true },
        { kind: 'drop', location: spec.to, guestIds: spec.guests.map((g) => g._id), at: completedAt, done: true }
      ]),
      route: {
        polyline: '',
        distanceMeters: Math.round(haversineKm(fromLL, toLL) * ROAD_FACTOR * 1000),
        durationSeconds: Math.round(legMin * 60),
        computedAt: startedAt,
        provider: 'osrm'
      },
      capacityUsed: { seats, luggage },
      deadlineAt: addMinutes(pickupAt, 45),
      assignmentMeta: {
        strategy,
        score: costBreakdown(legMin, { priorityTier: spec.guests[0].priorityTier, idleMin: idleBefore, capacityWaste: driver.capacity!.seats - seats, detourMin: detour }).total,
      ...
      timeline: [
        { at: startedAt, type: 'assigned', actor: 'engine', payload: { strategy } },
        { at: addMinutes(startedAt, 1), type: 'accepted', actor: 'driver', payload: {} },
        { at: pickupAt, type: 'boarded', actor: 'driver', payload: {} },
        { at: completedAt, type: 'completed', actor: 'driver', payload: {} }
      ],
      offeredAt: startedAt,
      acceptedAt: addMinutes(startedAt, 1),
      startedAt,
      completedAt,
      metrics: {
    ...
    await backdate(Trip, completedTrip._id, startedAt);
  ...
  for (const g of historyGuests) {
    await Guest.updateOne({ _id: g._id }, { $set: { status: 'completed', currentTripId: null, waitingSince: null } });
  }

  // ---------------- Live trips (Trip Board / Live Map / driver screen) -----
  const liveSpecs: Array<{ status: string; minutesAgo: number; progress: number; guest: GuestDoc; from: LocationDoc; to: LocationDoc }> = [
    { status: 'pending_driver', minutesAgo: 1, progress: 0, guest: liveGuests[0], from: airport, to: hotels[0] },
    { status: 'accepted', minutesAgo: 3, progress: 0.1, guest: liveGuests[1], from: station, to: hotels[1] },
    { status: 'en_route_pickup', minutesAgo: 8, progress: 0.55, guest: liveGuests[2], from: airport, to: hotels[2] },
    { status: 'at_pickup', minutesAgo: 14, progress: 1, guest: liveGuests[3], from: station, to: hotels[0] },
    { status: 'boarded', minutesAgo: 22, progress: 0.4, guest: liveGuests[4], from: airport, to: venue },
    { status: 'boarded', minutesAgo: 31, progress: 0.75, guest: liveGuests[5], from: hotels[1], to: venue }
  ];

  const driverStatusForTrip: Record<string, string> = {
  ...
  for (let i = 0; i < liveSpecs.length; i++) {
    ...
    const toLL = toLatLng(spec.to.coordinates as never);
    const legMin = driveMinutes(fromLL, toLL);
    const pickupEta = addMinutes(startedAt, Math.round(jitter(10, 3)));
    const dropEta = addMinutes(pickupEta, Math.round(legMin));
    ...
    const trip = await Trip.create({
      ...
      stops: buildStops([
        { kind: 'pickup', location: spec.from, guestIds: [spec.guest._id], at: pickupEta, done: boarded || spec.status === 'at_pickup' },
        { kind: 'drop', location: spec.to, guestIds: [spec.guest._id], at: dropEta, done: false }
      ]),
      route: {
        polyline: '',
        distanceMeters: Math.round(haversineKm(fromLL, toLL) * ROAD_FACTOR * 1000),
      ...
      capacityUsed: { seats: spec.guest.groupSize, luggage: spec.guest.luggageCount },
      deadlineAt: addMinutes(pickupEta, 40),
      assignmentMeta: {
        strategy: i === 2 ? 'detour_insert' : 'batch_hungarian',
        score: costBreakdown(legMin, { priorityTier: spec.guest.priorityTier, idleMin: 6, capacityWaste: driver.capacity!.seats - spec.guest.groupSize }).total,
        costBreakdown: costBreakdown(legMin, {
      ...
      timeline: [{ at: startedAt, type: 'assigned', actor: 'engine', payload: {} }],
      offeredAt: startedAt,
      acceptedAt: spec.status === 'pending_driver' ? null : addMinutes(startedAt, 1),
      startedAt: spec.status === 'pending_driver' ? null : addMinutes(startedAt, 1)
    });
    ...
    await Driver.updateOne(
      { _id: driver._id },
      {
        $set: {
          ...
          currentLocation: toGeoPoint(driverPos),
        ...
        $addToSet: { assignedTripIds: trip._id }
      }
    );

    await Guest.updateOne(
      { _id: spec.guest._id },
      { $set: { status: boarded ? 'in_transit' : 'assigned', currentTripId: trip._id, waitingSince: startedAt } }
    );

    await backdate(Trip, trip._id, startedAt);
    report.liveTrips++;
  }
  ...
  if (bigGroupGuest && bigGroupGuest.groupSize >= 12) {
    ...
    const fromLL = toLatLng(airport.coordinates as never);
    const toLL = toLatLng(hotels[0].coordinates as never);
    const legMin = driveMinutes(fromLL, toLL);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const trip = await Trip.create({
        ...
        stops: buildStops([
          { kind: 'pickup', location: airport, guestIds: [bigGroupGuest._id], at: addMinutes(startedAt, 11), done: false },
          { kind: 'drop', location: hotels[0], guestIds: [bigGroupGuest._id], at: addMinutes(startedAt, 11 + Math.round(legMin)), done: false }
        ]),
        route: { polyline: '', distanceMeters: Math.round(haversineKm(fromLL, toLL) * ROAD_FACTOR * 1000), durationSeconds: Math.round(legMin * 60), computedAt: startedAt, provider: 'osrm' },
        capacityUsed: { seats, luggage: Math.round(seats * 0.8) },
        deadlineAt: addMinutes(startedAt, 50),
        assignmentMeta: {
          strategy: 'batch_hungarian',
          score: costBreakdown(legMin, { priorityTier: bigGroupGuest.priorityTier, idleMin: 12, capacityWaste: driver.capacity!.seats - seats }).total,
          costBreakdown: costBreakdown(legMin, { priorityTier: bigGroupGuest.priorityTier, idleMin: 12, capacityWaste: driver.capacity!.seats - seats }),
          candidatesConsidered: 12,
          decidedAt: startedAt,
          decidedBy: 'engine'
        },
        groupSplitId,
        timeline: [{ at: startedAt, type: 'assigned', actor: 'engine', payload: { groupSplitId, leg: i + 1, of: 2 } }],
        offeredAt: startedAt,
        acceptedAt: addMinutes(startedAt, 1),
        startedAt: addMinutes(startedAt, 1)
      });
      ...
      await Driver.updateOne(
        { _id: driver._id },
        {
          $set: {
            status: 'en_route_pickup',
            currentTripId: trip._id,
            currentLocation: toGeoPoint(between(toLL, fromLL, 0.5)),
            locationUpdatedAt: new Date(now.getTime() - 4000),
            heading: 62,
            speedKmph: 41,
            predictedFreeAt: addMinutes(startedAt, 11 + Math.round(legMin)),
    ...
    await Guest.updateOne({ _id: bigGroupGuest._id }, { $set: { status: 'assigned', waitingSince: startedAt } });
  }
  ...
  const cancelledTrip = await Trip.create({
    ...
    vehicleSnapshot: { number: exceptionDriverA.vehicle!.number, model: exceptionDriverA.vehicle!.model, seats: exceptionDriverA.capacity!.seats, luggage: exceptionDriverA.capacity!.luggage },
    guests: [{ guestId: cancelledGuest._id, name: cancelledGuest.name, seats: 1, luggage: 1, pickupStopSeq: 0, dropStopSeq: 1 }],
    stops: buildStops([
      { kind: 'pickup', location: hotels[2], guestIds: [cancelledGuest._id], at: new Date(now.getTime() - 47 * 60_000), done: false },
      { kind: 'drop', location: venue, guestIds: [cancelledGuest._id], at: new Date(now.getTime() - 20 * 60_000), done: false }
    ]),
    capacityUsed: { seats: 1, luggage: 1 },
    cancelledAt,
    cancellationReason: 'Guest cancelled — joined a colleague already heading to the venue',
    timeline: [{ at: cancelledAt, type: 'cancelled', actor: 'guest', payload: {} }]
  });
  await backdate(Trip, cancelledTrip._id, new Date(now.getTime() - 47 * 60_000));
  ...
  const rejectedTrip = await Trip.create({
    ...
    vehicleSnapshot: { number: exceptionDriverB.vehicle!.number, model: exceptionDriverB.vehicle!.model, seats: exceptionDriverB.capacity!.seats, luggage: exceptionDriverB.capacity!.luggage },
    guests: [{ guestId: queueGuests[0]._id, name: queueGuests[0].name, seats: queueGuests[0].groupSize, luggage: queueGuests[0].luggageCount, pickupStopSeq: 0, dropStopSeq: 1 }],
    stops: buildStops([
      { kind: 'pickup', location: station, guestIds: [queueGuests[0]._id], at: new Date(now.getTime() - 26 * 60_000), done: false },
      { kind: 'drop', location: hotels[1], guestIds: [queueGuests[0]._id], at: new Date(now.getTime() - 5 * 60_000), done: false }
    ]),
    capacityUsed: { seats: queueGuests[0].groupSize, luggage: queueGuests[0].luggageCount },
    rejectedAt,
    rejectionReason: 'vehicle issue',
    timeline: [{ at: rejectedAt, type: 'rejected', actor: 'driver', payload: { reason: 'vehicle issue' } }]
  });
  await backdate(Trip, rejectedTrip._id, new Date(now.getTime() - 26 * 60_000));
  report.liveTrips += 2;
    ...
    await Driver.updateOne(
      { _id: driver._id },
      {
        $set: {
          status,
          currentTripId: null,
          currentLocation: toGeoPoint(between(toLatLng(spot.coordinates as never), toLatLng(venue.coordinates as never), 0.05 * n)),
          locationUpdatedAt: new Date(now.getTime() - 30_000),
          heading: Math.round(rand() * 359),
          speedKmph: 0,
          predictedFreeAt: status === 'on_break' ? addMinutes(now, 12) : now,
          'break.onBreakUntil': status === 'on_break' ? addMinutes(now, 12) : null,
  ...
  for (const [driverId, count] of driverTripCounts) {
    await Driver.updateOne(
      { _id: driverId },
      { $set: { 'stats.tripsCompleted': count, 'stats.guestsServed': count + Math.floor(rand() * 3), 'stats.totalDriveMinutes': count * 28, 'stats.totalIdleMinutes': count * 11 } }
    );
  }

  // ---------------- Ride requests awaiting approval -----------------------
  const requestSpecs = [
    { guest: requestGuests[0], from: hotels[0], to: venue, minutesAgo: 2, reason: 'Panel session at 14:00', passengers: 2 },
    { guest: requestGuests[1], from: hotels[1], to: airport, minutesAgo: 6, reason: 'Flight moved earlier — needs to leave now', passengers: 1 },
    { guest: requestGuests[2], from: venue, to: hotels[2], minutesAgo: 11, reason: 'Returning to hotel between sessions', passengers: 3 },
    { guest: requestGuests[3], from: hotels[2], to: station, minutesAgo: 19, reason: 'Collecting a colleague from the station', passengers: 1 }
  ];
  ...
  for (const spec of requestSpecs) {
    ...
    const request = await RideRequest.create({
      guestId: spec.guest._id,
      requestedAt,
      pickup: { locationId: spec.from._id, coordinates: spec.from.coordinates, label: spec.from.name },
      dropoff: { locationId: spec.to._id, coordinates: spec.to.coordinates, label: spec.to.name },
    ...
    await backdate(RideRequest, request._id, requestedAt);
  ...
  const queueSpecs = [
    { guest: queueGuests[0], from: station, to: hotels[1], waitedMin: 26, attempts: 5, reason: 'driver rejected (vehicle issue) · nearest alternative 31 min away' },
    { guest: queueGuests[1], from: airport, to: hotels[0], waitedMin: 17, attempts: 3, reason: '6 drivers on trips · 1 on break · nearest idle driver 24 min away' },
    { guest: queueGuests[2], from: airport, to: hotels[2], waitedMin: 9, attempts: 2, reason: 'awaiting next batch window' },
    { guest: queueGuests[3], from: venue, to: hotels[0], waitedMin: 3, attempts: 1, reason: '' }
  ];
  ...
  for (const spec of queueSpecs) {
    ...
    const entry = await QueueEntry.create({
      ...
      deadlineAt: addMinutes(enqueuedAt, 45),
      enqueuedAt,
      priorityTier: spec.guest.priorityTier,
    ...
    await backdate(QueueEntry, entry._id, enqueuedAt);
    await Guest.updateOne({ _id: spec.guest._id }, { $set: { status: 'queued', waitingSince: enqueuedAt } });
    report.queueEntries++;
  }
  ...
  const strandedEntry = await QueueEntry.create({
    type: 'ARRIVAL_PICKUP',
    guestIds: [strandedGuest._id],
    seats: 8,
    luggage: 10,
    pickup: { locationId: airport._id, coordinates: airport.coordinates, label: airport.name },
    dropoff: { locationId: hotels[1]._id, coordinates: hotels[1].coordinates, label: hotels[1].name },
    earliestAt: strandedEnqueuedAt,
    deadlineAt: addMinutes(strandedEnqueuedAt, 45),
  ...
  await backdate(QueueEntry, strandedEntry._id, strandedEnqueuedAt);
  await Guest.updateOne({ _id: strandedGuest._id }, { $set: { status: 'queued', waitingSince: strandedEnqueuedAt } });
  report.queueEntries++;

  // ---------------- Alerts -------------------------------------------------
  const alertSpecs: Array<{ level: 'info' | 'warning' | 'critical'; code: string; message: string; ack: boolean; minutesAgo: number }> = [
    { level: 'critical', code: 'STARVATION', message: `${queueGuests[0].name} has been waiting 26 min — past the ${20} min starvation threshold. Starvation sweep will force-match on the next pass.`, ack: false, minutesAgo: 2 },
    { level: 'warning', code: 'UNASSIGNABLE', message: 'No vehicle with 8 free seats — both tempo travellers are committed to the split-group convoy.', ack: false, minutesAgo: 5 },
    { level: 'warning', code: 'DEADLINE_RISK', message: 'Traffic multiplier raised to 1.4× for the 08:00–11:00 window; 3 arrival pickups are now projected to miss their 45-min deadline.', ack: false, minutesAgo: 12 },
    { level: 'info', code: 'DRIVER_BREAK', message: `${restDrivers[0]?.name ?? 'A driver'} hit 4 consecutive trips and was placed on a mandated 20-min break.`, ack: false, minutesAgo: 18 },
    { level: 'info', code: 'ROUTING_FALLBACK', message: 'OSRM returned 429 twice; circuit breaker opened and ETAs fell back to the haversine model for 90 s.', ack: true, minutesAgo: 42 },
    { level: 'info', code: 'GROUP_SPLIT', message: `Group of ${bigGroupGuest?.groupSize ?? 14} split across 2 tempo travellers and dispatched as a convoy.`, ack: true, minutesAgo: 13 }
  ];
  ...
  for (const a of alertSpecs) {
    ...
    await backdate(Alert, alert._id, raisedAt);
    report.alerts++;
  }

  // ---------------- Audit trail -------------------------------------------
  const auditSpecs = [
    { action: 'request.approve', entityType: 'RideRequest', minutesAgo: 8 },
    { action: 'trip.reassign', entityType: 'Trip', minutesAgo: 21 },
    { action: 'driver.suspend', entityType: 'Driver', minutesAgo: 37 },
    { action: 'config.update', entityType: 'EventConfig', minutesAgo: 52 },
    { action: 'queue.boost', entityType: 'QueueEntry', minutesAgo: 64 },
    { action: 'guest.import', entityType: 'Guest', minutesAgo: 180 },
    { action: 'dispatch.run', entityType: 'DispatchEngine', minutesAgo: 195 }
  ];
  ...
  logger.info(report, 'demo state seeded');
  return report;
}
