...
const client = _env.env.GEMINI_API_KEY ? new _generativeAi.GoogleGenerativeAI(_env.env.GEMINI_API_KEY) : null;
const GeminiService = {
  isEnabled() {
    return client !== null;
  },
  async ask(prompt) {
    ...
    resetBudgetIfNewDay();
    if (callsToday >= _env.env.AI_DAILY_CALL_BUDGET) {
      _logger.logger.warn('Gemini daily call budget exhausted');
      return null;
    }
