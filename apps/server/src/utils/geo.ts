...
export const toGeoPoint = (p: LatLng): GeoPoint => ({ type: 'Point', coordinates: [p.lng, p.lat] });

export const toLatLng = (g: GeoPoint): LatLng => ({ lat: g.coordinates[1], lng: g.coordinates[0] });

/** Leaflet wants [lat, lng] */
export const toLeafletTuple = (p: LatLng): [number, number] => [p.lat, p.lng];

/** OSRM wants "lng,lat;lng,lat" */
export const toOsrmCoords = (pts: LatLng[]): string => pts.map((p) => `${p.lng},${p.lat}`).join(';');
